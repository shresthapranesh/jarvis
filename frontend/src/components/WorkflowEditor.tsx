        ...
        <div className="wf-config-section">
          ...
          {showResilience && (
            <>
              ...
              <div className="wf-config-field"><label className="wf-config-label">Fallback output (JSON dict)</label><textarea className="wf-config-textarea" rows={3} value={typeof d.fallback_output === 'object' ? JSON.stringify(d.fallback_output, null, 2) : (d.fallback_output as string) || ''} onChange={(e) => { const parsed = safeJsonParse(e.target.value); if (parsed && typeof parsed === 'object') onUpdate({fallback_output: parsed}); else onUpdate({fallback_output: e.target.value as any}); }} placeholder='{"result": "default value"}' /></div>
            </>
          )}
        </div>
      )}
