...
function WorkflowEditorPage() {
  ...
  const {streaming, nodeStatuses, outputs, error: streamError} = useWorkflowStream(
    activeRunId,
    id,
  );
  ...
  async function handleSave(nodes: WorkflowRFNode[], edges: WorkflowRFEdge[]) {
    ...
    setSaveError(null);
    try {
      await updateWorkflow(id, {definition: serializeDefinition(nodes, edges)});
  ...
  async function doTriggerRun(inputs: Record<string, string>) {
    ...
    try {
      const {run_id} = await triggerWorkflowRun(id, inputs);
