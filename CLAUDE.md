...
│   ├── document_extractor.py  # PDF/DOCX/XLSX text extraction + is_tabular()/is_text_tabular()
...
│                         #   (SDK also has list_documents/document_path — the on-disk path, for code)
...
│   │                     #   (trafilatura extraction, then the real-browser rung)
...
`read(url)` climbs **two** rungs and stops at the first that returns real text: an httpx fetch + trafilatura, then a **persistent headed browser** attached over CDP (`browser=True`, or the older alias `js=True`, goes straight there). The browser rung exists for sites that turn automation away — the block is usually a headless fingerprint, a blank profile with no history, and a superhuman request rate, not the automation itself, so a real browser with a real profile is the fix rather than a disguise.

**There used to be a headless Chromium in between, and removing it was the point.** It answered *"the page renders client-side"* while being the *cause* of *"the site refused us"* — the same blank-profile fingerprint the third rung existed to escape. A page a plain fetch cannot read is one or the other, and the real browser answers both, so the middle rung only added a way to fail first. Two things follow: `read()` no longer imports Playwright at all (pinned by a test that parses the module), and **no downloaded browsers are needed** — `connect_over_cdp` speaks to one already installed, so `playwright install` is not part of setup. The regression that motivated it: the pinned Playwright build drifted ahead of the downloaded one, the rung failed on every call, and nothing noticed because the error was swallowed.
...
Config: `browser.cdp_url` / `browser.executable` / `browser.profile_dir` (Settings → Config, or env `JARVIS_BROWSER_*`). With the headless rung gone this is the only way to render a page, so a machine with no browser and no `DISPLAY`/`WAYLAND_DISPLAY` cannot read a client-side page locally — point `browser.cdp_url` at a browser elsewhere, which is the supported remote deployment: browser where the human is, jarvis on a server.
...
- **An attachment is a file on disk first, text second.** `AttachmentIn.document_path` is set beside `document_id` in `register_chat_task`, and *every* stub carries it, so the agent can open the bytes with `run_cell` instead of reading the file's text out of the prompt. `jarvis.document_path(id)` / `jarvis.list_documents()` (the SDK's `documents` category) recover a path the stub scrolled past. Sources that persist no `Document` row (bots, CLI) have no path and keep the inline behaviour. This existed as a column for a long time before it existed as a capability: `Document.path` was written by `register_chat_task` and then read by **nothing the agent could reach**, so `run_cell` — the agent's primary tool — had no way into an attachment, and the system prompt's own "Attached documents" section pointed away from it. A 100KB CSV was consequently walked into context, either inlined whole or through repeated `read_document` windows.
- **Tabular files never enter the conversation.** `_routes_to_code()` (`core/streaming.py`) catches csv/tsv/xlsx/parquet/jsonl with a persisted file and bypasses `_document_part` entirely — no extraction, no `start_indexing`. The message gets a path, a measured line count, and a five-line head (~400 chars for a 100KB file). **This is not a size optimization.** What anyone asks of a CSV is an *aggregate*, and neither pasted rows nor embedded chunks can produce one — top-k retrieval over six arbitrary row windows answers confidently and wrongly, which is worse than an error. Two rules the tests pin:
  - Extraction is **skipped, not discarded**. `_tabular_preview` reads a 64KB head, then streams the remainder only to count newlines — one sequential pass, constant memory — so a 10GB CSV costs what a 10MB one does. The old path decoded the whole file to a `str` purely to throw it away.
  - The line count and preview are stated **only when measured**. `is_text_tabular` is deliberately narrower than `is_tabular`: `.xlsx`/`.parquet` get the path with no head lines, because their first bytes are container framing, not rows. A guessed row count reads as fact.
- **Large documents are indexed, not inlined.** Documents whose extracted text exceeds `INLINE_THRESHOLD` (12k chars, `core/doc_index.py`) are chunked + embedded into `DocumentChunk` rows and replaced in the message by a stub carrying the `document_id`; the agent retrieves passages via the `search_documents`/`read_document` tools (`tools/documents.py`, conversation-scoped). Embeddings use a Gemini model (default `models/gemini-embedding-001`, override via `config set embedding.model <id>`; needs `GOOGLE_API_KEY`). Batches embed concurrently under a semaphore (`_EMBED_CONCURRENCY`, default 4, `JARVIS_EMBED_CONCURRENCY`). When embeddings are unavailable — or the document came from a source without a `Document` row (bots, CLI) — it falls back to inlining (capped at 80k chars), so nothing breaks on keyless/Ollama-only setups. Search is a single vectorized numpy matmul (`core/retrieval.py:cosine_ranking`) over the conversation's chunks; chunks cascade-delete with their Document. The inline fallback now **says when it truncated** and names the path — cutting at 80k silently is what let an answer be drawn, with no hedge, from the first 78% of a file.
